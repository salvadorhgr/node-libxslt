...
static xmlDoc* copyDocument(Local<Value> input) {
    libxmljs::XmlDocument* docWrapper =
        Nan::ObjectWrap::Unwrap<libxmljs::XmlDocument>(Nan::To<Object>(input).ToLocalChecked());
...
char** PrepareParams(Local<Array> array) {
...
NAN_METHOD(ApplySync) {
    ...
    bool outputString = (Nan::To<Boolean>(info[3]).ToLocalChecked())->Value();

    printf("outputString: %d\n", outputString);
    ...
    if (outputString) {
      ...
      libxmljs::XmlDocument* docResult = Nan::ObjectWrap::Unwrap<libxmljs::XmlDocument>(Nan::To<Object>(info[4]).ToLocalChecked());
...
void InitAll(Local<Object> exports) {
    ...
    Nan::Set(exports, Nan::New<String>("stylesheetSync").ToLocalChecked(),
        Nan::GetFunction(Nan::New<FunctionTemplate>(StylesheetSync)).ToLocalChecked());
    Nan::Set(exports, Nan::New<String>("applySync").ToLocalChecked(),
        Nan::GetFunction(Nan::New<FunctionTemplate>(ApplySync)).ToLocalChecked());
    Nan::Set(exports, Nan::New<String>("registerEXSLT").ToLocalChecked(),
        Nan::GetFunction(Nan::New<FunctionTemplate>(RegisterEXSLT)).ToLocalChecked());
}
